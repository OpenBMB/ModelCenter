            ...
            for data in f["data"]:
                for paragraph in data['paragraphs']:
                    for qa in paragraph['qas']:
                        input = " ".join(["Question:", qa["question"].lstrip(), "Answer: <extra_id_0>", "Context:", paragraph["context"].lstrip()])
                        if len(qa["answers"])==0:
                            qa["answers"] = [{"text": "no answer"}]
                        if split=='train':
                            target = " ".join(["<extra_id_0>", random.choice(qa["answers"])["text"], "<extra_id_1>"])
                        else:
