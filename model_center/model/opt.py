...
class OPT(BaseModel):
    ...
    def forward(self,
        ...
        Return:
            BaseModelOutput or tuple: The OPT output. 
        ...
        with torch.no_grad():
            ...
            if attention_mask.dim() == 2:
                if position_ids is None:
                    position_ids = (torch.cumsum(attention_mask, dim=1) * attention_mask).int() + 1
        ...
        # input the input embeddings into the OPT model
        current_key_values = None
        if use_cache:
            hidden_states, current_key_values = self.decoder(hidden_states, attention_mask, 
