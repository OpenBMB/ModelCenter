...
from .modeling_output import CausalLMOutputWithPast, BaseModelOutputWithPastAndCrossAttentions
...
class GPTj(BaseModel):
    ...
    def forward(self,
        ...
        with torch.no_grad():
            ...
            directional_mask_2d = torch.arange(seq_length, device=device) <= torch.arange(seq_length,
                                                                                          device=device).view(-1, 1)
            attention_mask = attention_mask.view(batch, 1, seq_length) & directional_mask_2d.view(1, seq_length,
                                                                                                  seq_length)
...
class GPTjForLM(BaseModel):
    ...
    def forward(self,
        ...
        if not return_dict:
            return (loss, logits, outputs.past_key_values, hidden_states, outputs.attentions)
