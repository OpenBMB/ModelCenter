    ...
    def forward(self,
        ...
        if encoder_outputs is None:
            ...
            with torch.no_grad():
                if attention_mask is not None:
                    ...
                    attention_mask = torch.arange(seq_enc, device=device)[
                        None, :].repeat(batch, 1) < length[:, None]
                # (batch, seq_enc, seq_enc)
                enc_attention_mask = attention_mask.view(
                    batch, seq_enc, 1) & attention_mask.view(batch, 1, seq_enc)
            ...
            encoder_outputs = self.encoder(
                hidden_states_enc, enc_attention_mask, enc_position_bias)
        ...
        with torch.no_grad():
            if decoder_attention_mask is not None:
                ...
                decoder_attention_mask = torch.arange(seq_length, device=device)[
                    None, :].repeat(batch, 1) < decoder_length[:, None]
            directional_mask_2d = torch.arange(seq_length, device=device) <= torch.arange(
                seq_length, device=device).view(-1, 1)
            # (batch, seq_dec, seq_dec)
            dec_attention_mask = decoder_attention_mask.view(batch, seq_length, 1) & decoder_attention_mask.view(
                batch, 1, seq_length) & directional_mask_2d.view(1, seq_length, seq_length)
            # (batch, seq_dec, seq_enc)
            cross_attention_mask = attention_mask.view(
                batch, 1, seq_enc) & decoder_attention_mask.view(batch, seq_length, 1)
        ...
        if use_cache:
            decoder_outputs, current_key_values=self.decoder(hidden_states_dec, dec_attention_mask, dec_position_bias, encoder_outputs, cross_attention_mask, None,
        ...
        if return_logits:
            return logits, hidden_states_dec, current_key_values  # *(100*self.config.dim_model**-0.5)
