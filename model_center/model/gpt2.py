...
class GPT2ForLM(BaseModel):
    ...
    def forward(self,
        ...
        if labels:
            ...
            # TODO:检查一下fusedCrossEntropy是不是就是
            _logits = logits[..., :-1, :].contiguous()
            _labels = labels[..., 1:].contiguous()
            loss = loss_func(_logits.view(-1, _logits.size(-1)), labels.view(-1))

        if not return_dict:
            return (loss, logits, outputs.past_key_values, hidden_states, outputs.attentions, outputs.cross_attentions)
