...
class T5Generation:
    def __init__(self, model, tokenizer):
        ...
        self.end_tokens = [self.tokenizer.additional_special_tokens_ids[1], self.tokenizer.eos_token_id]
    ...
    def _convert_to_tensors(self, input_text):
        ...
        model_inputs["decoder_input_ids"] = [self.tokenizer.pad_token_id, self.tokenizer.additional_special_tokens_ids[0]] # t5 use pad_token_id as decode start
        ...
        **kwargs
        ...
        for i in range(max_length + 1):
            if i == 0:
                ...
                logits, past_key_values, encoder_outputs = out.logits, out.past_key_values, out.encoder_last_hidden_state
            else:
                out = self.model(
...
class T5RandomSampling(T5Generation):
    def _decode(
        self,
        model_inputs,
        max_length=100,
        ...
        **kwargs
        ...
        for i in range(max_length):
            ...
            for idx in range(batch_size):
                if not done[idx] and (
                    next_token[idx].item() in self.end_tokens or i == max_length - 1
                ):
                    done[idx] = True
                    results[idx] = decoder_input_ids[idx, pred_start_index:].clone().cpu().tolist()  # type: ignore # noqa: E501
