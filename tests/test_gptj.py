...
def main():
    ...
    length = torch.ones((batch, ), dtype=torch.int32).cuda() * max_encoder_length
    attention_mask = torch.arange(input_ids.shape[1], device=input_ids.device)[None, :].repeat(input_ids.shape[0], 1) < length[:, None]

    bmt_logits = bmt_gptj(input_ids = input_ids, attention_mask = attention_mask, return_logits=True)
