...
from model_center.model import RobertaForLM

from transformers import RobertaForMaskedLM as hugRoberta
...
def main():
    ...
    config.dropout_p = 0
    bmt_roberta = RobertaForLM.from_pretrained(path, config=config)
    ...
    for _ in range(10):
        ...
        labels = torch.arange(input_ids.shape[1], device=input_ids.device)[None, :].repeat(input_ids.shape[0], 1) < length[:, None]
        bmt_logits = bmt_roberta(input_ids = input_ids, attention_mask = attention_mask,labels=labels).prediction_logits
