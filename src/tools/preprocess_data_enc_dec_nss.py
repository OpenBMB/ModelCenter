...
class Encoder(object):
    ...
    def encode(self, all_lines):
        ...
        for i, sent_ids in enumerate(doc_ids):
            ...
            n_easy_doc = [doc_ids[x][:86] for x in n_easy]

            n_hard_doc = []
            for neg in neg_doc_ids:
                r = random.randint(0, len(neg) - 1)
                n_hard_doc.append(neg[r][:86])
            ...
            pair = sent_ids[:389] + [Encoder.tokenizer.get_sentinel_id(0)]
            for i, option in enumerate(options):
                pair.extend([number_map[i], 20] + option + [18])
            pairs.append(pair)
            labels.append([1, Encoder.tokenizer.get_sentinel_id(0)] + [number_map[label]] + [Encoder.tokenizer.get_sentinel_id(1)])
