...
def get_args():
    ...
    group.add_argument('--input', default="/mnt/sfs_turbo/new_data/backup/merge_", type=str, help='Path to input TXT')
    
    group = parser.add_argument_group(title='tokenizer')
    group.add_argument('--tokenizer_path', default="/mnt/sfs_turbo/hx/CPM-1/vocab/new_cn/", type=str, help='Path of tokenizer')

    group = parser.add_argument_group(title='output data')
    group.add_argument("--output_path", default="/mnt/sfs_turbo/hx/CPM-1/new_data/", type=str)
...
def main():
    ...
    res = []
    for _, (pair_ids, _, _) in enumerate(encoded_docs, start=1):
        if pair_ids is None:
            continue
        res = res + pair_ids
    random.shuffle(res)
