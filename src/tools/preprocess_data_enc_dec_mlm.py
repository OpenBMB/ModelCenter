...
class Encoder(object):
    ...
    def encode(self, line):
        ...
        span_start_ends = self.random_spans_noise_mask(
            len(data), noisy_density=self.args.noisy_density, mean_noise_span_length=self.args.mean_noise_span_length)
        start = 0
        no_noise_tokens = []
        noise_tokens = []
        final = ""
        ...
        for i, span in enumerate(span_start_ends):
            ...
            no_noise_tokens.append(Encoder.tokenizer.encode(data[start:span[0]]) + [0])
            noise_tokens.append([0] + Encoder.tokenizer.encode(data[span[0]:span[1]])) # "[0]+" placeholder for sentinel
            final = final + data[start:span[0]] + "____(" + data[span[0]:span[1]] + ")"
            start = span[1]
        noise_tokens[-1].append(Encoder.tokenizer.eod_id)
...
def get_args():
    ...
    group.add_argument('--input', default="/mnt/sfs_turbo/hx/CPM-2.1/raw_data/pretrain_raw_10g_shuf.txt", type=str, help='Path to input TXT')
    ...
    group.add_argument('--output_prefix', default="new_mlm", type=str,
