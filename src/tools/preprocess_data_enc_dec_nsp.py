...
class Encoder(object):
    ...
    def encode(self, line):
        ...
        for i, sent_ids in enumerate(doc_ids):
            ...
            pair = sent_ids + [Encoder.tokenizer.get_sentinel_id(0)] + doc_ids[r]
            pairs.append(pair)
            # labels.append([1, Encoder.tokenizer.get_sentinel_id(0)] + [label] + [Encoder.tokenizer.get_sentinel_id(1)])
            labels.append([1, Encoder.tokenizer.get_sentinel_id(0)] + [label])
...
def main():
    ...
    encoded_docs = pool.imap_unordered(encoder.encode, fin, 1)
